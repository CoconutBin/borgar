...
chooseIceCream.addEventListener('change', () => {
  if ((chooseIceCream as HTMLInputElement).checked) {
...
icecreamDiv.addEventListener('submit', function (event) {
  ...
  let scoopCount = (document.getElementById('scoopcount') as HTMLInputElement).value;
  let toppingsInput = (document.getElementById('icecreamtoppings') as HTMLInputElement).value;
  let flavorsInput = (document.getElementById('icecreamflavors') as HTMLInputElement).value;
  ...
  if (icecreamOrder.scoops == 0) {
    ...
    if (elapsedTime > 10000) icecreamOrder.melt()
